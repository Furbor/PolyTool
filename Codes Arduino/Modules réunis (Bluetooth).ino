...
// Bibliothèque pour le Pulse sensor :
...
// Préfixe pour une utilisation simplifiée du module Bluetooth :
...
// Autres variables :
  ...
  // Pour la photorésistance
    ...
    String c1 = "Endroit peu eclaire";
    String c2 = "Endroit sombre !!";
  // Pour le LOGO-sensor :
...
void loop() { 
  ...
  case DHT_SUCCESS: 
    ...
    BlueT.print(prefixe_T+F("Temperature (Celsius) : ")+temperature);
    break;
 
  case DHT_TIMEOUT_ERROR: 
    //Serial.println(F("Pas de réponse !"));
    BlueT.print(prefixe_H+F("Pas de reponse !"));
    break;
 
  case DHT_CHECKSUM_ERROR: 
    //Serial.println(F("Problème de communication !")); 
    BlueT.print(prefixe_H+F("Probleme de communication !"));
    break;
  }
  // Pour la photorésistance :
  int luminosite = analogRead(capteur_photo);
  //Serial.println(luminosite); // Vérification sur le moniteur série
  if (luminosite<120) {
    //Serial.println(c0); // Vérification sur le moniteur série
    BlueT.print(prefixe_texte + c0); // Ecriture sur l'appareil connecté en Bluetooth 
  }
  if (luminosite>120 && luminosite<=350) {
    //Serial.println(c1); // Vérification sur le moniteur série
    BlueT.print(prefixe_texte + c1); // Ecriture sur l'appareil connecté en Bluetooth
  }
  if (luminosite>350) {
    //Serial.println(c2); // Vérification sur le moniteur série
...
void compteur(){
  delay(delai);
  tour=tour+1;  
  //Serial.print("Tour n°"); // Vérification sur le moniteur série
  //Serial.println(int(tour)); // Vérification sur le moniteur série
}
